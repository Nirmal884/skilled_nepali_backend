const MedicalCentreModel = require('../models/medicalCentre.model');
const { uploadToS3 } = require('../utils/s3Uploader');

const MedicalCentreService = {
    async createMedicalCentre(data, files) {
        const {
            name,
            description,
            status,
        ...
        if (!name || !name.trim()) {
            throw new Error('Medical centre name is required');
        }
        ...
        const createData = {
            ...
            status: status?.trim() || 'GCC Approved (GAMCA)',
        ...
        return await MedicalCentreModel.create(createData);
    },

    async getAllMedicalCentresAdmin(query) {
        const { page, limit, search, status, isActive } = query;
        return await MedicalCentreModel.findAllAdmin({
    ...
    async getMedicalCentresPublic(query) {
        const { page, limit, search, district, province, status } = query;
        return await MedicalCentreModel.findAllPublic({
            page: page ? Number(page) : 1,
    ...
    async getMedicalCentreById(id) {
        const centre = await MedicalCentreModel.findById(id);
        if (!centre) {
            throw new Error('Medical centre not found');
        }
        return centre;
    },

    async updateMedicalCentre(id, data, files) {
        const existing = await MedicalCentreModel.findById(id);
        if (!existing) {
            throw new Error('Medical centre not found');
        }
        ...
        return await MedicalCentreModel.update(id, updateData);
    },

    async deleteMedicalCentre(id) {
        const existing = await MedicalCentreModel.findById(id);
        if (!existing) {
            throw new Error('Medical centre not found');
        }
        return await MedicalCentreModel.softDelete(id);
    }
};

module.exports = MedicalCentreService;
